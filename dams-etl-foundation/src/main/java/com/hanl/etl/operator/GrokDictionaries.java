...
import com.hanl.etl.shaded.com.google.code.regexp.Pattern;
import com.hanl.etl.shaded.com.google.common.io.Closeables;
...
public final class GrokDictionaries {
  ...
  public GrokDictionaries(Config config, Configs configs) {
    ...
    try {
      ...
      throw new OperatorCompilationException("Cannot compile grok dictionary", config, e);
    }
  ...
  private void loadDictionary(Reader reader) throws IOException {
    for (String line : CharStreams.readLines(reader)) {
      ...
      int i = line.indexOf(" ");
      if (i < 0) {
        throw new OperatorCompilationException("Dictionary entry line must contain a space to separate name and value: " + line, getConfig());
      }
      if (i == 0) {
        throw new OperatorCompilationException("Dictionary entry line must contain a name: " + line, getConfig());
      }
      String name = line.substring(0, i);
      String value = line.substring(i + 1, line.length()).trim();
      if (value.length() == 0) {
        throw new OperatorCompilationException("Dictionary entry line must contain a value: " + line, getConfig());
      }
  ...
  private String resolveExpression(String expr) {
    ...
    while (true) {
      ...
      if (refValue == null) {
        throw new OperatorCompilationException("Missing value for name: " + regexName, getConfig());
      }
